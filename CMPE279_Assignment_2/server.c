...
int main(int argc, char const *argv[])
{
    ...
    char *hello = "Hello from server";
     ...
     //compare child process is the one running
...
//Attaching socket to port 80
    ...
    //drop privileges wrapper before message processing
    if(drop_privileges()){
         //message processing
            ...
            //Modification for assignment 2
            
            int CopyParent = dup(new_socket);
            
            if(CopyParent==-1)
           		perror("socket duplicating function failed");
...
//Simply done to add delay so that child process can complete execution
    wait();
    return 0;
}
